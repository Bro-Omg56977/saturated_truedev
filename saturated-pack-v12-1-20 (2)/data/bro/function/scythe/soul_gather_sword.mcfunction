...
scoreboard objectives add soul_gather_sword_chance dummy
execute store result score @s soul_gather_sword_chance run random value 1..20

# adds tag and plays sfx

execute if score @s soul_gather_sword_chance matches 1 run tag @s[tag=!soulgathereffect] add soulgathereffect
execute if score @s soul_gather_sword_chance matches 1 run playsound minecraft:entity.zombie.attack_iron_door ambient @s[tag=soulgathereffect]
