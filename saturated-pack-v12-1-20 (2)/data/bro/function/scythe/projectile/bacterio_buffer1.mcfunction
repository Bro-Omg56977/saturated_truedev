...
execute as @e[tag=bacterioarrow,tag=!bacteriosource,tag=bacterioentity] at @s run tag @e[tag=!bacterioarrow,tag=!bacteriosource,tag=!bacterioentity,distance=..20] add bacterioentity
execute as @e[tag=bacterioarrow,tag=!bacteriosource] at @s run effect give @s minecraft:weakness 5 5
execute as @e[tag=bacteriosource] run schedule function bro:scythe/projectile/bacterio_buffer2 1s replace