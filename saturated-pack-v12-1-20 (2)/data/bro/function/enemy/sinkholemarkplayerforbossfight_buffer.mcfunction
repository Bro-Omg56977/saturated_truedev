...
execute as @e[tag=satsinkholebossminion] at @s run scoreboard players set @e[distance=..25] sat.sinkholeboss.0 1

#if the score is 1 then repeat function in 2 seconds
execute as @e[tag=satsinkholebossplayer] at @s run schedule function bro:enemy/sinkholemarkplayerforbossfight_buffer0 2s replace
