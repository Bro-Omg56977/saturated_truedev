...
execute as @e[type=marker,tag=sat.superelec.auradecor,tag=!sat.superelec.auradecor.modified] at @s run summon area_effect_cloud ~ ~.25 ~ {Particle:{type:electric_spark},Radius:1,RadiusPerTick:0.03,Duration:60,potion_contents:{potion:wither,custom_effects:[{id:wither,duration:40,amplifier:1,show_particles:1b,show_icon:1b}]}}
...
execute as @e[type=marker,tag=sat.superelec.auradecor] at @s run particle end_rod ~ ~ ~ 0 0 0 0.01 15 force @a[distance=..34]
